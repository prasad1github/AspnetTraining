 <%@ Page Title="" Language="C#" MasterPageFile="~/Main.Master" AutoEventWireup="true" CodeBehind="WebForm1.aspx.cs" EnableEventValidation="false" Inherits="WebApplicationTrainingDemo.WebForm1" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                ...
                <tbody>
                    ...
                    <tr>
                        <td align="center" colspan="2">
                            ...
                            <asp:Button ID="btnClose" runat="server" Text="Close" CausesValidation="False" class="buttonclass"  style="background-color:#d9534f" OnClientClick="return closePopup();" /></td>
    ...
    <script>
        function clearFields() {
            ...
            $("#<%=txtEmpDescription.ClientID%>").val('');
        ...
        function showPopup(empId, empFirstname, empMiddlename, empLastname, empGender, empContactno, empcity, empDescription, empUsername, empPassword, companyId) {
            //debugger;
            $("#<%=txtFirstname.ClientID%>").val(empFirstname);
            $("#<%=txtMiddlename.ClientID%>").val(empMiddlename);
            $("#<%=txtLastname.ClientID%>").val(empLastname);
            if (empGender == 'Male')
                $("#<%=ddlGender.ClientID%>").val(1);
            else
                $("#<%=ddlGender.ClientID%>").val(2);
            $("#<%=txtContactno.ClientID%>").val(empContactno);
            $("#<%=txtCity.ClientID%>").val(empcity);
            $("#<%=txtEmpDescription.ClientID%>").val(empDescription);
            $("#<%=txtUsername.ClientID%>").val(empUsername);
            $("#<%=txtPassword.ClientID%>").val(empPassword);
            $("#<%=txtConfirmPassword.ClientID%>").val(empPassword);
            //$('id$="lblId"').val(empId);            
            $("#<%=hdnid.ClientID%>").val(empId);

            //alert($("#hdnid").text());
            showUpdate();
            HideSumbit();
            var showpopup = $find("<%=addPopupModal.ClientID%>");
            showpopup.show();
            return false;
        }
